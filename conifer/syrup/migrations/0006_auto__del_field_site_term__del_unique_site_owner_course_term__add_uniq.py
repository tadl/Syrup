...
class Migration(SchemaMigration):
    ...
    def backwards(self, orm):
        ...
        for site in orm.Site.objects.all():
            site.term = site.start_term
            site.save()

        # Adding unique constraint on 'Site', fields ['owner', 'course', 'term']
        db.create_unique('syrup_site', ['owner_id', 'course_id', 'term_id'])

        # Removing unique constraint on 'Site', fields ['owner', 'course', 'start_term']
        db.delete_unique('syrup_site', ['owner_id', 'course_id', 'start_term_id'])
